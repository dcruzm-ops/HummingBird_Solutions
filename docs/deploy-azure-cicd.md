...
SQL de verificación rápida (antes de vistas):

```sql
SELECT
  OBJECT_ID('dbo.PlanesPago', 'U') AS PlanesPago,
  OBJECT_ID('dbo.CuotasPago', 'U') AS CuotasPago,
  OBJECT_ID('dbo.TransaccionesPago', 'U') AS TransaccionesPago;
```

Si alguno viene `NULL`, re-ejecutar `Azure/01_creacion_tablas_azure_safe.sql` y luego `Azure/02_creacion_vistas_reportes_azure_safe.sql`.
